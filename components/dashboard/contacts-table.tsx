      ...
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {contacts.map((contact) => (
            <tr key={contact.id} className="border-b border-stone-100 align-top hover:bg-stone-50/60">
              <td className="py-3 pr-4">
                <p className="font-medium text-slate-950">
                  <Link href={`/dashboard/members/${contact.id}`} className="text-teal-900 hover:text-slate-950">
                    {contact.firstName} {contact.lastName}
                  </Link>
                </p>
                <p className="text-slate-600">{contact.email}</p>
                <p className="text-xs text-slate-500">{contact.phone ?? "No phone"}</p>
                <p className="mt-1 font-mono text-[10px] text-slate-400">{tenantSlug}</p>
              </td>
              <td className="py-3 pr-4">
                ...
                <p className="mt-1.5 text-xs text-slate-600">
                  <span className="font-medium text-slate-700">Channel:</span> {contactSourceLabel(contact.source)}
                </p>
              ...
              <td className="py-3 pr-4">
                <p className="text-slate-950">{contact.lastActivitySummary ?? "-"}</p>
                <p className="mt-0.5 text-xs text-slate-500" title={formatDate(contact.lastActivityAt ?? contact.createdAt)}>
                  {formatRelativeTime(contact.lastActivityAt ?? contact.createdAt)}
                </p>
              </td>
              <td className="py-3 pr-4">
                {contact._count.registrations === 0 ? (
                  <p className="text-xs text-slate-500">No event registrations</p>
                ) : (
                  <>
                    <p className="font-medium text-slate-950">
                    ...
                    <ul className="mt-1 space-y-1">
                      {contact.registrations.map((registration) => (
                        <li key={registration.id} className="text-xs text-slate-600">
                          <span className="font-medium text-slate-800">{registration.event.title}</span>
                          {" / "}
                          {registration.status}
                          {" / "}
                    ...
                    {contact._count.registrations > contact.registrations.length && (
                      <p className="mt-1 text-[10px] text-slate-400">Showing latest {contact.registrations.length}</p>
                    )}
                  </>
                )}
              </td>
              <td className="py-3 pr-4 min-w-[200px]">
                {contact.tags.length > 0 && (
                  <div className="mb-2 flex flex-wrap gap-1">
                    {contact.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full bg-stone-100 px-2 py-0.5 text-[10px] font-medium text-slate-700"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {contact.notes ? (
                  <p className="mb-2 max-w-[280px] whitespace-pre-wrap text-xs text-slate-600">{contact.notes}</p>
                ) : (
                  <p className="mb-2 text-xs text-slate-400">No admin notes</p>
                )}
                <details className="mt-1">
                  <summary className="cursor-pointer text-xs font-semibold text-teal-900">Edit contact</summary>
                  <form action={updateContactAction} className="mt-2 grid gap-2 rounded border border-stone-200 bg-stone-50 p-2">
                    ...
                    <input
                      ...
                      className="rounded border border-stone-300 px-2 py-1 text-xs"
                    />
                    <select name="type" defaultValue={contact.type} className="rounded border border-stone-300 px-2 py-1 text-xs">
                    ...
                    <textarea
                      ...
                      className="rounded border border-stone-300 px-2 py-1 text-xs"
                    />
                    <button type="submit" className="rounded bg-slate-950 px-2 py-1 text-xs text-white hover:bg-teal-900">
                      Save changes
                    </button>
                  </form>
