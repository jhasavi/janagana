      ...
      <header className="border-b border-stone-200/80 bg-[#fffaf2]/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <nav className="flex flex-wrap items-center gap-1 text-sm">
            <Link href={`/portal/${tenant.slug}`} className="inline-flex items-center gap-1.5 rounded-md px-3 py-2 font-medium text-slate-700 hover:bg-white hover:text-slate-950">
              <UsersRound className="h-4 w-4" />
              <span>Home</span>
            </Link>
            <Link href={`/portal/${tenant.slug}/events`} className="inline-flex items-center gap-1.5 rounded-md px-3 py-2 font-medium text-slate-700 hover:bg-white hover:text-slate-950">
              <CalendarDays className="h-4 w-4" />
              <span>Events</span>
            </Link>
            <Link href={`/portal/${tenant.slug}/join`} className="inline-flex items-center gap-1.5 rounded-md px-3 py-2 font-medium text-slate-700 hover:bg-white hover:text-slate-950">
              <HeartHandshake className="h-4 w-4" />
              <span>Join</span>
            </Link>
            <Link href={`/portal/${tenant.slug}/contact`} className="inline-flex items-center gap-1.5 rounded-md bg-slate-950 px-3 py-2 font-medium text-white hover:bg-teal-900">
